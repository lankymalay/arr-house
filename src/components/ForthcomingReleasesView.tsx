...
import type { ExternalReleaseItem, ForthcomingReleasesPayload, MediaItem, SearchResultItem, ServiceId } from '../types.js';
...
interface ForthcomingReleasesViewProps {
  ...
  onAddedItem?: () => void;
  isEmbedded?: boolean;
  activeType?: 'movie' | 'tv' | 'music';
  onTypeChange?: (type: 'movie' | 'tv' | 'music') => void;
}
...
export const ForthcomingReleasesView: React.FC<ForthcomingReleasesViewProps> = ({ 
  ...
  // Filters
  const [internalActiveType, setInternalActiveType] = useState<'movie' | 'tv' | 'music'>('movie');
  const activeType = propActiveType || internalActiveType;
  const handleTypeChange = (type: 'movie' | 'tv' | 'music') => {
  ...
  }, [activeType, selectedMonthKey, sortBy]);
  ...
  }, [processedAllItems, activeType, selectedMonthKey, sortBy]);
  ...
  const getServiceForType = (type: 'tv' | 'movie' | 'music'): ServiceId => {
    switch (type) {
  ...
  const handleOpenAddModal = (item: ExternalReleaseItem) => {
    const service = getServiceForType(item.mediaType);
    const year = item.date ? parseInt(item.date.split('-')[0], 10) : undefined;
    const cleanForeignId = (item.id && !item.id.startsWith('tentpole-') && !item.id.startsWith('wiki-')) ? item.id : '';
    ...
    const searchItem: SearchResultItem = {
      ...
      title: item.title,
      year,
      authorOrArtist: item.seriesOrArtistTitle,
    ...
    setSelectedItemForAdd(searchItem);
  };
  ...
  return (
    <div className={`animate-fadeIn ${isEmbedded ? 'space-y-4' : 'p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-4'}`}>
      {/* Timeline & Sort Bar */}
      <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-2.5 bg-[#14171f]/60 border border-white/[0.06] rounded-xl text-xs">
        {/* Month Selector Pills */}
        <div className="flex flex-wrap items-center gap-1.5 py-0.5">
          <span className="text-slate-500 mr-1 flex items-center gap-1 shrink-0 font-medium">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center justify-center p-1.5 rounded-lg text-xs font-semibold bg-[#141822] hover:bg-white/[0.08] border border-white/[0.08] hover:border-cyan-500/30 text-slate-300 hover:text-white transition shadow-sm active:scale-95 disabled:opacity-50 cursor-pointer shrink-0"
      ...
      {loading ? (
        ...
        <div className="bg-[#0d1016] border border-white/[0.08] rounded-2xl p-12 text-center space-y-3">
          ...
          <p className="text-xs text-slate-400 max-w-sm mx-auto">
            Try clearing your month filter or switching media category.
          </p>
          <button
            onClick={() => {
                ...
                >
                  ...
                  <div className="p-3.5 flex-1 flex flex-col justify-between space-y-2">
                    <div>
                      ...
                      <ExpandableSynopsis
                        text={item.overview}
                        fallbackText="No extended overview provided for this release."
                        maxWords={50}
                        className="text-xs text-slate-300/90 mt-1.5 leading-relaxed"
                      />
                    </div>

                    {/* Action Area: Genres & Grab Buttons */}
                      ...
                      <div className="w-full">
                        {(() => {
                          ...
                          const inLibrary = (libraryItems?.some(
                            lib => (lib.title.toLowerCase() === item.title.toLowerCase() || (item.id && String(lib.foreignId || lib.id) === String(item.id))) && lib.service === service
                          )) || addedIds.has(item.id || item.title);
                          const isAdding = addingIds.has(item.id || item.title);

                          if (inLibrary) {
                            return (
                              <div 
                                className="w-full justify-center px-3 py-1.5 rounded-full bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 text-xs font-semibold flex items-center gap-1.5 shadow-sm"
                                title={`Already in ${service.toUpperCase()}`}
                              >
                          ...
                          return (
                            <button
                              id={`btn-forthcoming-add-${item.id}`}
                              type="button"
                              onClick={(e) => {
                                e.stopPropagation();
                              ...
                              disabled={isAdding}
                              className="w-full justify-center px-3 py-1.5 rounded-full bg-white hover:bg-neutral-200 text-black text-xs font-bold flex items-center gap-1.5 shadow transition-all cursor-pointer pixel-pill active:scale-[0.98] disabled:opacity-50 whitespace-nowrap"
                              title={`Add "${item.title}" to ${service.toUpperCase()}`}
                            >
                              {isAdding ? (
                                <>
      ...
      {selectedItem && (
        ...
        >
          ...
          >
            ...
            {/* Poster Header */}
            <div className="relative aspect-[2/3] max-h-80 bg-slate-900 overflow-hidden">
              <MediaPoster
            ...
            <div className="p-6 space-y-5">
              ...
              {/* Action Buttons */}
              <div className="pt-3 border-t border-white/[0.08] flex items-center justify-between gap-3 flex-wrap">
                <button
                  onClick={() => setSelectedItem(null)}
                ...
                <div>
                  {(() => {
                    const service = getServiceForType(selectedItem.mediaType);
                    const inLibrary = (libraryItems?.some(
                      lib => (lib.title.toLowerCase() === selectedItem.title.toLowerCase() || (selectedItem.id && String(lib.foreignId || lib.id) === String(selectedItem.id))) && lib.service === service
                    )) || addedIds.has(selectedItem.id || selectedItem.title);
                    const isAdding = addingIds.has(selectedItem.id || selectedItem.title);

                    if (inLibrary) {
                      return (
                        <div className="px-4 py-2 rounded-full bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 text-xs font-semibold flex items-center gap-1.5 shadow-sm">
                          <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
                          <span>Already in {service.toUpperCase()}</span>
                        </div>
                      );
                    }
                    ...
                    return (
                      <button
                        id={`btn-modal-add-${selectedItem.id}`}
                        type="button"
                        onClick={() => handleOpenAddModal(selectedItem)}
                        disabled={isAdding}
                        className="px-5 py-2 rounded-full bg-white hover:bg-neutral-200 text-black text-xs font-bold flex items-center gap-1.5 shadow transition-all cursor-pointer pixel-pill active:scale-[0.98] disabled:opacity-50"
                        title={`Add to ${service.toUpperCase()}`}
                      >
                        {isAdding ? (
                          <>
      ...
      {/* Download Quality Profile Selection Modal */}
      {selectedItemForAdd && (
        <AddContentModal
          item={selectedItemForAdd}
          onClose={() => setSelectedItemForAdd(null)}
          onAdded={() => {
            const key = selectedItemForAdd.foreignId || selectedItemForAdd.title;
            setAddedIds(prev => new Set(prev).add(key));
            setSelectedItemForAdd(null);
            if (selectedItem && (selectedItem.id === selectedItemForAdd.foreignId || selectedItem.title === selectedItemForAdd.title)) {
