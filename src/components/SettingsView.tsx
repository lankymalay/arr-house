...
export const SettingsView: React.FC<SettingsViewProps> = ({ onRefreshStack, user: propUser, servicesStatus = [] }) => {
  ...
  return (
    <div className="p-3.5 sm:p-6 lg:p-8 space-y-5 sm:space-y-6 max-w-7xl mx-auto">
      ...
      {/* 4 Stack Services Live Status Panel (Moved from Header to Settings) */}
      <div className="bg-[#121824] border border-[#2d3c54] rounded-2xl p-4 sm:p-5 shadow-sm">
        <div className="flex items-center justify-between mb-3 px-1">
          ...
          <button
            ...
            title="Refresh stack status"
            className="text-xs text-indigo-400 hover:text-white flex items-center gap-1.5 px-3 py-1 rounded-full hover:bg-white/[0.06] transition-colors cursor-pointer pixel-pill font-medium"
          >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-3">
          {(['sonarr', 'radarr', 'lidarr', 'prowlarr'] as ServiceId[]).map((id) => {
            ...
            const isConnected = liveStatus?.status === 'connected' || svc?.status === 'connected';
            const isError = liveStatus?.status === 'error' || svc?.status === 'error';
